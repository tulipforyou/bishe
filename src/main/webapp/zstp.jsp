...
<nav class="navbar navbar-dark fixed-top bg-dark flex-md-nowrap p-0 shadow">
    ...
    <a href="${pageContext.request.contextPath}/zstp.jsp"  class="form-control form-control-dark w-100">课程图谱</a>
        ...
        <div class="row">
            ...
            <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">知识图谱</h1>
                    <form id="fileUpload1" method="post" action="zstp" enctype="multipart/form-data">
                        <input type="file" class="btn btn-outline-success" name="upload">
                        <input type="submit" class="btn btn-outline-primary" value="图谱展示"/>
                    </form>
                </div>

                <c:if test="${empty zstp}">
                    <div class="container-fluid">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <table class="table table-bordered">
                                    ...
                                    <tr>
                                        ...
                                        <th scope="col">三级</th>
                                        <th scope="col">四级</th>
                                        <th scope="col" style="width: 110px">五级</th>
                                        <th scope="col">六级</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        <c:forEach items="${zstp}" var="map">
                                            <c:if test="${map.key.length() == 2}">
                                                <tr>
                                                    <td>${map.value}</td> <td></td> <td></td> <td></td> <td></td> <td></td>
                                                </tr>
                                            </c:if>
                                            <c:if test="${map.key.length() == 3}">
                                                <tr>
                                                    <td></td> <td>${map.value}</td> <td></td> <td></td> <td></td> <td></td>
                                                </tr>
                                            </c:if>
                                            <c:if test="${map.key.length() == 4}">
                                                <tr>
                                                    <td></td> <td></td> <td>${map.value}</td> <td></td> <td></td> <td></td>
                                                </tr>
                                            </c:if>
                                            <c:if test="${map.key.length() == 5}">
                                                <tr>
                                                    <td></td> <td></td> <td></td> <td>${map.value}</td> <td></td> <td></td>
                                                </tr>
                                            </c:if>
                                            <c:if test="${map.key.length() == 6}">
                                                <tr>
                                                    <td></td> <td></td> <td></td> <td></td> <td>${map.value}</td> <td></td>
                                                </tr>
                                            </c:if>
                                            <c:if test="${map.key.length() == 7}">
                                                <tr>
                                                    <td></td> <td></td> <td></td> <td></td> <td></td> <td>${map.value}</td>
                                                </tr>
                                            </c:if>
                                        </c:forEach>
                                    </tbody>
